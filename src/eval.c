...
void eval_open(const char *file)
{
	...
	if (sizeof(int) != 4)
		fatal_error("int size is not compatible with Edax.\n");
	//	-(unsigned) short are 16 bits
	if (sizeof(short) != 2)
		fatal_error("short size is not compatible with Edax.\n");
	...
	if (r != 2 || (!(edax_header == EDAX || eval_header == EVAL) && !(edax_header == XADE || eval_header == LAVE)))
		fatal_error("%s is not an Edax evaluation file\n", file);
	r = fread(&version, sizeof(int), 1, f);
	r += fread(&release, sizeof(int), 1, f);
	r += fread(&build, sizeof(int), 1, f);
	r += fread(&date, sizeof(double), 1, f);
	if (r != 4)
		fatal_error("Cannot read version info from %s\n", file);
	if (edax_header == XADE)
	{
	...
	for (ply = 0; ply < EVAL_N_PLY; ply++)
	{
		...
		if (r != n_w)
			fatal_error("Cannot read evaluation weight from %s\n", file);
		if (edax_header == XADE)
			for (i = 0; i < n_w; ++i)
				w[i] = bswap_short(w[i]);
		i = j = offset = 0;
...
static void eval_update_0(Eval *eval, const Move *move)
{
	...
	{
	case 7:
		feature[s->feature[6].i] -= 2 * s->feature[6].x;
	case 6:
		feature[s->feature[5].i] -= 2 * s->feature[5].x;
	case 5:
		feature[s->feature[4].i] -= 2 * s->feature[4].x;
	case 4:
		feature[s->feature[3].i] -= 2 * s->feature[3].x;
	case 3:
	...
	foreach_bit(x, f)
	{
		...
		{
		case 7:
			feature[s->feature[6].i] -= s->feature[6].x;
		case 6:
			feature[s->feature[5].i] -= s->feature[5].x;
		case 5:
			feature[s->feature[4].i] -= s->feature[4].x;
		case 4:
			feature[s->feature[3].i] -= s->feature[3].x;
		case 3:
